...
echo '[INFO][dhall-utility-library][refreeze.sh] Refreezing...'
...
for depth in $(seq "${deepest_level}" -1 1) ; do
  while IFS= read -r -d '' dhall_file ; do
    echo "[INFO][dhall-utility-library][refreeze.sh] Freezing: ${dhall_file} ..."
    dhall freeze --cache --all --inplace "${dhall_file}"
  done < <(find . -mindepth "${depth}" -maxdepth "${depth}" -name 'package.dhall' -print0)
done
echo '[INFO][dhall-utility-library][refreeze.sh] Finished refreezing successfully!'
